...
-- Type synonym of `foreign import render` from below, to simplify
-- `start'`'s type signature.
...
-- Wrapping input in a string in lines 65-67 of Pux.js make this a
-- `Html a` renderer instead of a `forall target. target a` renderer.
...
type Config state action eff = Config' Html state action eff
...
type App state action = App' Html state action
